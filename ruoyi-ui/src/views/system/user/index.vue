...
<!--          </el-form-item>-->
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
          ...
          <el-col :span="24">
            <el-form-item label="用户类型">
              <el-select style="width: 100%;" v-model="form.userType" placeholder="请选择用户类型">
                <el-option v-for="item in dict.type.sys_user_type" v-if="item.value !== '00'" :key="item.value"
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="所属医院">
              <el-select style="width: 100%;" v-model="form.hospitalId" placeholder="请选择医院">
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="所属干休所">
              <el-select style="width: 100%;" v-model="form.retiredId" placeholder="请选择干休所">
                <el-option v-for="item in retiredOptions" :key="item.value" :label="item.label"
                  :value="item.value"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
...
<script>
import { listHospital } from "@/api/examine/hospital";
import { listRetired } from "@/api/examine/retired";
...
export default {
  ...
  methods: {
    ...
    // 查询医院列表
    getHospitalOption() {
      listHospital({ pageNum: 1, pageSize: 999 }).then(response => {
        this.hospitalOptions = response.rows.map((item, index, arr) => {
    ...
    // 查询干休所列表
    getRetiredOption() {
      listRetired({ pageNum: 1, pageSize: 999 }).then(response => {
        this.retiredOptions = response.rows.map((item, index, arr) => {
          let c = { label: item.name, value: item.id }
          return c;
        })
      })
